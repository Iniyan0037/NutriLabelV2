...
export default function ManualInputScreen({ route, navigation }) {
  ...
  const [ocrNutrition, setOcrNutrition] = useState(null);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      navigation.navigate('Results', { apiResult: result, selectedProfiles, ingredientText: ingredientText.trim(), nutrition: ocrNutrition, productImageUrl: productImageUri });
    } catch (error) {
  ...
  const processOCRImage = async (imageSource) => {
    ...
    setOcrNutrition(response.nutrition || null);
    const nutritionFound = response.nutrition && Object.keys(response.nutrition).some((key) => key !== 'data_source');
    Alert.alert('OCR Complete', nutritionFound ? 'Ingredient text and possible nutrition values were extracted. Please review them before analysing.' : 'Ingredient text has been extracted. Please review it before analysing.');
  };
  ...
  const handleImageSelection = async (mode) => {
    try {
      ...
      setProductImageUri(originalAsset.uri || '');
      const preparedImage = await compressImageForOCR(originalAsset);
