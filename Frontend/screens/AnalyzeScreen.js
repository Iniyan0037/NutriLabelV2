...
export default function AnalyzeScreen({ navigation }) {
  ...
  return (
    <AppScaffold navigation={navigation} current="Analyze" title="Analyze food">
      {({ colors }) => (
        <>
          <Text style={[styles.intro, { color: colors.muted }]}>Choose how you want to check a product. Results, history and saved logs stay linked to the selected profile.</Text>

          <ActionRow
            colors={colors}
            icon="camera-outline"
            title="Scan barcode or OCR label"
            text="Use the camera for packaged products or ingredient labels."
          ...
          <ActionRow
            colors={colors}
            icon="create-outline"
            title="Type ingredients manually"
            text="Paste an ingredient list when scanning is not clear."
...
function ActionRow({ colors, icon, title, text, onPress, disabled }) {
  return (
    <MotionPressable disabled={disabled} onPress={onPress} scaleTo={1.03} style={[styles.row, { backgroundColor: colors.card, borderColor: colors.border }]}> 
