...
function missionFor(profile) {
  if (profile === GENERAL_PROFILE) {
    return { profile, title: 'Find hidden caution cards for general label awareness' };
  }
  return {
    profile,
...
function cardIsRiskyForMission(card, profile) {
  if (profile === GENERAL_PROFILE) return card.tags.length > 0;
  return card.tags.includes(profile);
}
...
function stableShuffle(list, seed) {
  return [...list].sort((a, b) => `${a.id}-${seed}`.localeCompare(`${b.id}-${seed}`));
}

function pickRoundCards(profile, round) {
  const risky = rotate(CARDS.filter((card) => cardIsRiskyForMission(card, profile)), round).slice(0, RISK_CARDS_PER_ROUND);
  const safe = rotate(CARDS.filter((card) => !cardIsRiskyForMission(card, profile)), round * 3).slice(0, SAFE_CARDS_PER_ROUND);
  return stableShuffle([...risky, ...safe], round);
}
...
export default function LearningGameScreen({ navigation }) {
  ...
  const mission = missionFor(profileList.length ? profileList[round % profileList.length] : GENERAL_PROFILE);
  const roundCards = useMemo(() => pickRoundCards(mission.profile, round), [mission.profile, round]);
  const riskyIds = useMemo(() => roundCards.filter((card) => cardIsRiskyForMission(card, mission.profile)).map((card) => card.id), [roundCards, mission.profile]);
        ...
        <>
          <View style={[styles.hero, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.subtitle, { color: colors.muted }]}>Memorise the cards, then choose only the hidden cards that are risky for today’s mission.</Text>
            </View>
          </View>
          ...
          {stage === 'start' ? <Panel colors={colors} title="Why play today?">
            <Text style={[styles.body, { color: colors.muted }]}>{profileList.length ? 'Each round uses one restriction from your active profile. This helps you recognise risky ingredients faster before shopping or eating.' : 'This profile has no restrictions selected, so today’s round focuses on general label awareness and common caution ingredients.'}</Text>
          ...
          {stage === 'memorise' ? <>
            ...
            <View style={styles.grid}>{roundCards.map((card) => <VisibleCard key={card.id} colors={colors} card={card} profile={mission.profile} />)}</View>
          </> : null}

          {stage === 'recall' ? <>
            <Panel colors={colors} title="Cards are hidden">
              <Text style={[styles.mission, { color: colors.text }]}>{mission.title}</Text>
              <Text style={[styles.body, { color: colors.muted }]}>Select the hidden cards you remember as risky.</Text>
            </Panel>
            ...
            <MotionPressable disabled={!selected.length} style={[styles.primary, { backgroundColor: colors.primary, opacity: selected.length ? 1 : 0.55 }]} onPress={() => setStage('feedback')}><Text style={[styles.primaryText, { color: colors.primaryText }]}>Check answer</Text></MotionPressable>
          </> : null}

          {stage === 'feedback' ? <Panel colors={colors} title="Round result">
            <Text style={[styles.celebration, { color: colors.text }]}>{score === riskyIds.length ? '🎉 Excellent work!' : score >= Math.ceil(riskyIds.length / 2) ? '👏 Nice try — keep learning!' : '🌱 Better luck next time!'}</Text>
            <Text style={[styles.score, { color: colors.primary }]}>Score {score}/{riskyIds.length}</Text>
            <Text style={[styles.body, { color: colors.muted }]}>{score === riskyIds.length ? 'You found every risky card for this mission.' : score >= Math.ceil(riskyIds.length / 2) ? `You found ${score} of ${riskyIds.length} risky cards. A quick review will help next round.` : `You found ${score} of ${riskyIds.length} risky cards. Try again to build confidence with labels.`}</Text>
            {missed > 0 ? <Text style={[styles.body, { color: colors.muted, marginTop: 6 }]}>Review the missed risky ingredients below.</Text> : null}
            {roundCards.map((card) => {
              const isRisk = riskyIds.includes(card.id);
              const chosen = selected.includes(card.id);
              if (!isRisk && !chosen) return null;
              const label = isRisk && chosen ? 'correct' : isRisk && !chosen ? 'missed' : 'not risky for this mission';
              const icon = isRisk && chosen ? '✅' : isRisk && !chosen ? '⚠️' : '❌';
              return <View key={card.id} style={[styles.feedbackRow, { borderTopColor: colors.border }]}><Text style={[styles.feedbackName, { color: colors.text }]}>{icon} {card.name} — {label}</Text><Text style={[styles.body, { color: colors.muted }]}>{card.reason}</Text></View>;
            })}
...
function Panel({ colors, title, children }) {
  return <View style={[styles.panel, { backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.panelTitle, { color: colors.text }]}>{title}</Text>{children}</View>;
}

function Stat({ colors, value, label }) {
  return <View style={[styles.stat, { backgroundColor: colors.card, borderColor: colors.border }]}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72} style={[styles.statValue, { color: colors.primary }]}>{value}</Text><Text style={[styles.statLabel, { color: colors.muted }]}>{label}</Text></View>;
}

function Instruction({ colors, number, text }) {
  return <View style={styles.instructionRow}><View style={[styles.instructionCircle, { backgroundColor: colors.primary }]}><Text style={[styles.instructionNumber, { color: colors.primaryText }]}>{number}</Text></View><Text style={[styles.instructionText, { color: colors.muted }]}>{text}</Text></View>;
}

function VisibleCard({ colors, card, profile }) {
  const risky = cardIsRiskyForMission(card, profile);
  return <View style={[styles.gameCard, { backgroundColor: colors.card, borderColor: risky ? colors.primary : colors.border }]}><Text style={styles.emoji}>{card.emoji}</Text><Text style={[styles.cardName, { color: colors.text }]}>{card.name}</Text><Text style={[styles.cardStatus, { color: risky ? colors.primary : colors.muted }]}>{risky ? `Risk for ${displayProfile(profile)}` : 'Not risky for this mission'}</Text></View>;
}

function HiddenCard({ colors, index, selected, onPress }) {
  return <MotionPressable onPress={onPress} scaleTo={1.025} style={[styles.gameCard, { backgroundColor: selected ? colors.card2 : colors.card, borderColor: selected ? colors.primary : colors.border }]}><Text style={[styles.questionMark, { color: colors.primary }]}>?</Text><Text style={[styles.cardName, { color: colors.text }]}>Card {index + 1}</Text><Text style={[styles.cardStatus, { color: selected ? colors.primary : colors.muted }]}>{selected ? 'Selected' : 'Hidden'}</Text></MotionPressable>;
}
...
const styles = StyleSheet.create({
  ...
  stat: { flex: 1, minHeight: 76, borderWidth: 1, borderRadius: 20, padding: 10, alignItems: 'center', justifyContent: 'center' },
  statValue: { fontSize: 17, fontWeight: '900', textAlign: 'center' },
  statLabel: { fontSize: 10, fontWeight: '900', textTransform: 'uppercase', marginTop: 3, textAlign: 'center' },
  ...
  instructionRow: { flexDirection: 'row', gap: 10, marginTop: 10, alignItems: 'center' },
  instructionCircle: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  instructionNumber: { fontWeight: '900', lineHeight: 18, textAlign: 'center' },
  instructionText: { flex: 1, fontWeight: '800', lineHeight: 21 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 16 },
  gameCard: { width: '47%', height: 150, borderRadius: 22, padding: 12, alignItems: 'center', justifyContent: 'center', borderWidth: 2 },
  emoji: { fontSize: 42 },
  questionMark: { fontSize: 54, fontWeight: '900' },
  cardName: { fontWeight: '900', textAlign: 'center', marginTop: 8, fontSize: 14 },
  ...
  celebration: { fontSize: 20, fontWeight: '900', marginBottom: 8 },
