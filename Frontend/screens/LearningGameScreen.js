...
function pickRoundCards(profile, round) {
  const risky = rotate(CARDS.filter((card) => card.tags.includes(profile)), round).slice(0, RISK_CARDS_PER_ROUND);
  const safe = rotate(CARDS.filter((card) => !card.tags.includes(profile)), round * 3).slice(0, SAFE_CARDS_PER_ROUND);
...
export default function LearningGameScreen({ navigation }) {
  ...
  const riskyIds = useMemo(() => roundCards.filter((card) => card.tags.includes(mission.profile)).map((card) => card.id), [roundCards, mission.profile]);
      ...
      {({ colors }) => (
        <>
          ...
          {stage === 'start' ? <Panel colors={colors} title="Why play today?">
            <Text style={[styles.body, { color: colors.muted }]}>Each round uses one restriction from your active profile. This helps you recognise risky ingredients faster before shopping or eating.</Text>
          ...
          {stage === 'feedback' ? <Panel colors={colors} title="Round result">
            ...
            <Text style={[styles.body, { color: colors.muted }]}>{score === riskyIds.length ? 'You found every risky card for this mission.' : `You found ${score} of ${riskyIds.length} risky cards.`}</Text>
