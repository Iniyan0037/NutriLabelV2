    ...
    <LinearGradient colors={colors.gradient} style={styles.flex}>
      <SafeAreaView style={styles.flex}>
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={[styles.barcodeHero, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            <View style={styles.barcodeLines}>
              {[5, 10, 4, 14, 7, 11, 5, 13, 8, 4, 12].map((w, i) => (
                <View key={i} style={[styles.barcodeLine, { width: w, backgroundColor: colors.primary }]} />
              ))}
            </View>
            <Text style={[styles.barcodeHeroTitle, { color: colors.text }]}>Barcode scanner</Text>
            <Text style={[styles.barcodeHeroText, { color: colors.muted }]}>Use this page only for product barcodes. Ingredient-label OCR is available from Manual input.</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  barcodeHero: { borderRadius: 22, borderWidth: 1, padding: 16, marginBottom: 16, alignItems: 'center' },
  barcodeLines: { height: 74, flexDirection: 'row', alignItems: 'stretch', justifyContent: 'center', gap: 4, marginBottom: 10 },
  barcodeLine: { height: 74, borderRadius: 2 },
  barcodeHeroTitle: { fontSize: 20, fontWeight: '900' },
  barcodeHeroText: { fontSize: 13, lineHeight: 19, textAlign: 'center', fontWeight: '700', marginTop: 4 },
