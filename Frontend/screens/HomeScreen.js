      ...
      {({ colors }) => (
        <>
          <View style={[styles.hero, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            ...
            <Text style={[styles.heroSub, { color: colors.muted }]}>NutriLabel checks ingredients, allergens, additives and nutrition against the selected dietary profile.</Text>
          </View>
          ...
          <View style={[styles.profileCard, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            <View style={styles.cardHeader}>
              ...
              <MotionPressable onPress={goToProfile} style={[styles.smallBtn, { backgroundColor: colors.primary }]}> 
          ...
          <View style={[styles.planCard, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            ...
            <Step colors={colors} number="1" text={hasProfile ? 'Warm up with the profile memory game.' : 'Create or select your dietary profile.'} />
            <Step colors={colors} number="2" text={hasProfile ? 'Check the next packaged food before deciding.' : 'Return to the home screen once a profile is active.'} />
            <Step colors={colors} number="3" text={hasProfile ? 'Log the item only if you eat it.' : 'Use the bottom tabs to analyse, learn and track.'} />
          </View>
        </>
      )}
    </AppScaffold>
...
function Step({ colors, number, text }) {
  return (
    <View style={styles.stepRow}>
      <Text style={[styles.stepNumber, { backgroundColor: colors.primary, color: colors.primaryText }]}>{number}</Text>
...
const styles = StyleSheet.create({
  ...
  stepNumber: { width: 28, height: 28, borderRadius: 14, textAlign: 'center', textAlignVertical: 'center', paddingTop: 4, fontWeight: '900' },
  stepText: { flex: 1, fontSize: 14, lineHeight: 20, fontWeight: '800' },
  logoutFull: { minHeight: 48, borderRadius: 18, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 14 },
  logoutText: { flexShrink: 1, fontWeight: '900' },
});
