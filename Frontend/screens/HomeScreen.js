...
export default function HomeScreen({ navigation, route }) {
  ...
  const logout = async () => {
    const confirmed = Platform.OS === 'web' ? window.confirm('Log out of this family login?') : true;
    if (!confirmed) return;
  ...
  const requireProfile = (screen) => {
    if (!hasProfile) {
      const msg = 'Please create or select a profile first so the feature can stay tied to one family member.';
      ...
      <SafeAreaView style={styles.flex}>
        <ScrollView contentContainerStyle={styles.container}>
          <View style={styles.topLine}>
            <Text style={[styles.accountText, { color: colors.muted }]}>Family login: {account?.family_name || 'Unknown'}</Text>
