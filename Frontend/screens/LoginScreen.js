...
export default function LoginScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    if (!familyName.trim() || pin.trim().length < 4) {
      showMessage('Details required', 'Enter a shared login name and a PIN with at least 4 characters.');
      return;
    }
    try {
      setLoading(true);
    ...
    <LinearGradient colors={['#FAFDF8', '#F5FAF0', '#EAF5DE']} style={styles.flex}>
      <SafeAreaView style={styles.flex}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
          ...
          <Text style={styles.title}>{mode === 'login' ? 'Shared Login' : 'Create Shared Login'}</Text>
          <Text style={styles.subtitle}>One shared login can hold up to six separate dietary profiles. History and food logs stay tied to the selected profile.</Text>

          <TextInput style={styles.input} value={familyName} onChangeText={setFamilyName} placeholder="Shared login name" placeholderTextColor="#6F806B" autoCapitalize="words" />
          <TextInput style={styles.input} value={pin} onChangeText={setPin} placeholder="Shared login PIN" placeholderTextColor="#6F806B" secureTextEntry />
          ...
          <Pressable style={styles.switchButton} onPress={() => setMode(mode === 'login' ? 'register' : 'login')}>
            <Text style={styles.switchText}>{mode === 'login' ? 'Need a new shared login? Create one' : 'Already have a login? Sign in'}</Text>
          </Pressable>
