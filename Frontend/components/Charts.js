...
export function LineChart({ colors, data, valueKey = 'value', labelKey = 'label', selectedLabel, onPointPress }) {
  const width = 292;
  const height = 148;
  const pad = 20;
  ...
  return (
    <View style={styles.lineWrap}>
      <View style={[styles.plot, { width, height, backgroundColor: colors.card2, borderColor: colors.border }]}> 
        ...
        {points.map((p, index) => {
          const selected = String(selectedLabel || '') === String(p.item[labelKey]);
          const DotComponent = onPointPress ? Pressable : View;
          return (
            <DotComponent
              key={p.item[labelKey]}
              onPress={onPointPress ? () => onPointPress(p.item) : undefined}
              style={[styles.dot, selected && styles.dotSelected, { left: p.x - (selected ? 9 : 6), top: p.y - (selected ? 9 : 6), backgroundColor: chartColor(colors, index), borderColor: selected ? colors.primary : colors.card }]}
            />
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  dotSelected: { width: 18, height: 18, borderRadius: 9, borderWidth: 3 },
