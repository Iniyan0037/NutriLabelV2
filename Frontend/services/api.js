...
export const CLIENT_STORAGE_VERSION = '2026-05-iteration3-final-patch';
export const CLIENT_VERSION_KEY = 'NUTRILABEL_CLIENT_VERSION';
...
async function requestJson(path, options = {}) {
  ...
  if (account?.auth_token) headers['X-Account-Token'] = account.auth_token;

  const response = await fetch(`${BASE_URL}${path}`, { ...options, headers });
  const parsed = await parseResponse(response);
  if (!response.ok) {
    const message = buildError(parsed, `${options.method || 'GET'} ${path} failed: ${response.status}`);
    if (response.status === 401 && String(message).toLowerCase().includes('valid account token')) {
      await clearStoredAccount();
      throw new Error('Session expired. Please sign in again so the app can create a fresh account token.');
    }
...
export async function getStoredAccount() {
  ...
  if (account?.client_storage_version !== CLIENT_STORAGE_VERSION) {
    await clearStoredAccount();
    return null;
  }
  return account;
...
export async function setStoredAccount(account) {
  await AsyncStorage.setItem(ACCOUNT_KEY, JSON.stringify({ ...account, client_storage_version: CLIENT_STORAGE_VERSION }));
  await AsyncStorage.setItem(CLIENT_VERSION_KEY, CLIENT_STORAGE_VERSION);
}
...
export async function clearStoredAccount() {
  ...
  const keysToRemove = keys.filter((key) =>
    ...
    key === PROFILE_KEY ||
    key === CLIENT_VERSION_KEY ||
    key === 'HISTORY' ||
    key === 'HIGH_CONTRAST' ||
    key.startsWith('NUTRILABEL_') ||
    key.startsWith('GAME_STREAK_') ||
    key.startsWith('TIPS_DONE_')
  );
...
export function normalizeProfile(profile) {
  ...
  const map = {
    ...
    'no restriction': '', 'no restrictions': '', 'no preference': '', 'no preferences': '', unrestricted: '', none: '',
  };
...
export function displayProfile(profile) {
  ...
  const map = { vegan: 'Vegan', vegetarian: 'Vegetarian', eggetarian: 'Eggetarian', halal: 'Halal', Jain: 'Jain', 'nut-free': 'Nut Free', 'dairy-free': 'Dairy Free', 'gluten-free': 'Gluten Free', kosher: 'Kosher', general: 'General label awareness' };
  return map[normalized] || String(profile || 'No restrictions');
}
...
export async function saveProfile(profileName, restrictions, explanation = {}) {
  ...
  const parsed = await requestJson('/profile/save', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ account_id: accountId, profile_name: profileName, restrictions: normalizeProfiles(restrictions), explanation }),
  });
  return parsed;
}
