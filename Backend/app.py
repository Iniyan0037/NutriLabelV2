...
from db import init_db, db_status_counts, SessionLocal, Profile, History, UserAccount, FoodLog, NutritionGoal, AllergenInfo, AwarenessTip, ENumber, Rule
...
def register_account():
    ...
    if len(family_name) < 2:
        return jsonify({"error": "family_name must be at least 2 characters"}), 400
    if len(pin) < 4:
    ...
    try:
        ...
        if existing:
            return jsonify({"error": "This family login already exists. Please sign in instead."}), 409
...
def login_account():
    ...
    try:
        ...
        if not account or not check_password_hash(account.pin_hash, pin):
            return jsonify({"error": "Invalid family name or PIN"}), 401
...
def save_profile():
    ...
    if not account_id:
        return jsonify({"error": "account_id is required so profiles stay tied to one family login"}), 400
    ...
    try:
        if db.query(Profile).filter(Profile.account_id == account_id).count() >= 6:
            return jsonify({"error": "A family login can only have a maximum of six profiles."}), 400
...
def save_history():
    ...
    if not account_id:
        return jsonify({"error": "account_id is required so history stays tied to one family login"}), 400
