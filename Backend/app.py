...
allowed_origins = (
    ...
    or "https://nutrilabel.live,https://www.nutrilabel.live"
)

if allowed_origins == "*":
    CORS(app, allow_headers=["Content-Type", "X-Account-Token", "X-Admin-Token"])
else:
    origins = [origin.strip().rstrip("/") for origin in allowed_origins.split(",") if origin.strip()]
    CORS(app, resources={r"/*": {"origins": origins}}, allow_headers=["Content-Type", "X-Account-Token", "X-Admin-Token"])

MAX_CACHE_ITEMS = 100
MAX_IMAGE_BYTES = 1_500_000
ALLOWED_OCR_MIME_TYPES = {"image/jpeg", "image/png", "image/webp"}
ALLOWED_OCR_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp"}
...
APP_AUTH_SECRET = (
    ...
    or os.getenv("ADMIN_DEBUG_TOKEN")
    or "development-secret-change-in-render"
)
...
def issue_account_token(account):
    payload = {
    ...
    signature = hmac.new(APP_AUTH_SECRET.encode("utf-8"), payload_b64.encode("utf-8"), hashlib.sha256).hexdigest()
    return f"{payload_b64}.{signature}"


def verify_account_token(token, expected_account_id=None):
    if not token or "." not in str(token):
        return None
    try:
        payload_b64, signature = str(token).split(".", 1)
        expected_signature = hmac.new(APP_AUTH_SECRET.encode("utf-8"), payload_b64.encode("utf-8"), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(signature, expected_signature):
            return None
        payload = json.loads(_b64url_decode(payload_b64).decode("utf-8"))
        account_id = int(payload.get("account_id"))
        if expected_account_id is not None and account_id != int(expected_account_id):
            return None
        issued_at = int(payload.get("iat", 0))
        if issued_at <= 0 or int(time.time()) - issued_at > AUTH_TOKEN_TTL_SECONDS:
            return None
        return payload
    except Exception:
        return None
...
def get_account_token_from_request(data=None):
    data = data or {}
    return request.headers.get("X-Account-Token") or data.get("auth_token") or request.args.get("auth_token")


def require_account_context(data=None):
    account_id = get_account_id_from_request(data)
    if not account_id:
        return None, (jsonify({"error": "account_id is required"}), 400)
    token = get_account_token_from_request(data)
    if not verify_account_token(token, account_id):
        return None, (jsonify({"error": "Valid account token is required"}), 401)
    return account_id, None


def require_admin_token():
    if os.getenv("ENABLE_PUBLIC_DB_STATUS", "").lower() == "true":
        return True
    expected_token = os.getenv("ADMIN_DEBUG_TOKEN")
...
def validate_ocr_file(file):
    ...
    if extension not in ALLOWED_OCR_EXTENSIONS:
        return False, "Unsupported file extension. Use JPG, PNG or WEBP."
    if file.mimetype not in ALLOWED_OCR_MIME_TYPES:
...
def home():
    return jsonify({
        ...
        "status": "ok",
        "version": "iteration-3-security-hardened",
    })
...
def get_product(barcode):
    ...
    off_url = (
        ...
        "image_front_url,image_url,image_small_url"
    )
...
def extract_text_from_image():
    ...
    is_valid_file, validation_error = validate_ocr_file(image_file)
    ...
    if isinstance(religious, list):
        for item in religious:
            ...
            if value == "kosher":
                add_profile(
                    "kosher",
                    "Selected because the user chose kosher dietary requirements.",
                )
...
def register_account():
    ...
    try:
        ...
        db.commit()
        db.refresh(account)
        return jsonify({"id": account.id, "family_name": account.family_name, "auth_token": issue_account_token(account)}), 201
    finally:
        db.close()
...
def login_account():
    ...
    try:
        ...
        return jsonify({"id": account.id, "family_name": account.family_name, "auth_token": issue_account_token(account)})
    finally:
        db.close()
...
def save_profile():
    ...
    account_id, auth_error = require_account_context(data)
    if auth_error:
        return auth_error
...
def get_profiles():
    account_id, auth_error = require_account_context()
    if auth_error:
        return auth_error
    db = SessionLocal()
    try:
        profiles = db.query(Profile).filter(Profile.account_id == account_id).order_by(Profile.created_at.desc()).all()
...
def update_profile(profile_id):
    ...
    db = SessionLocal()
    try:
        profile = db.query(Profile).filter(Profile.id == profile_id, Profile.account_id == account_id).first()
        if not profile:
...
def get_history():
    ...
    db = SessionLocal()
    try:
        query = db.query(History).filter(History.account_id == account_id)
        if profile_id:
        ...
        return jsonify([{
            ...
            "product_image_url": (item.analysis_json or {}).get("product_image_url") or (item.analysis_json or {}).get("image_url") or "",
...
def get_history_detail(history_id):
    ...
    db = SessionLocal()
    try:
        item = db.query(History).filter(History.id == history_id, History.account_id == account_id).first()
        if not item:
...
def recommendations():
    ...
    if "kosher" in profiles:
        add_recommendation("check", "gelatin, shellfish, uncertified animal derivatives and dairy/meat combinations", "These may require kosher certification verification.")
...
def delete_food_log(log_id):
    ...
    db = SessionLocal()
    try:
        item = db.query(FoodLog).filter(FoodLog.id == log_id, FoodLog.account_id == account_id).first()
        if not item:
